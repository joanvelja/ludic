...
async def mint_sneaky_data(
    ...
    rollout_generator: Callable[[str, str], Rollout],
    current_round: int,
    ...
    async def generate_one(sample: Dict[str, Any]) -> Optional[RolloutRecord]:
    ...
    tasks = [generate_one(sample) for sample in samples[:config.max_samples]]
